...
class NavigationAppLive extends StatefulWidget {
  const NavigationAppLive({super.key});

  @override
  State<NavigationAppLive> createState() => _NavigationAppLiveState();
}

class _NavigationAppLiveState extends State<NavigationAppLive>
  ...
  final List<ArcGISPoint> _trackingPoints = [];

  late StreamSubscription _locationSubscription;

  ArcGISPoint? _currentPosition;
  ...
  Future<void> initRoute() async {
    ...
    if (_currentPosition == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Current position not yet available."),
    ...
    final initialLocation = Stop(_currentPosition!);
    ...
    try {
      ...
      routeParameters.preserveFirstStop = false;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Error computing route: $e"),
          backgroundColor: Colors.red,
        ),
      );
    }
  ...
  Future<void> startNavigation() async {
    if (!_routeGraphicsOverlay.graphics.contains(_remainingRouteGraphic)) {
      _routeGraphicsOverlay.graphics.add(_remainingRouteGraphic);
    }
